...
#include <ws2tcpip.h>     // beinhaltet spezifische Information fr Winsock2 wzB. IP_HDRINCL
...
int session_values_MySQL(unsigned char *datatcp, int lentcp, SESS *sess_data, char client_ip[], unsigned short int client_port, char server_ip[], unsigned short int server_port, char * timestamp);
...
int is_mysql_packet(char *RecvBuf, struct transaction_packet *trans_packet); // Listens the socket and returns an Oracle packet
...
//TODO change to int Thin_Client=0;
int Thin_Client = 1;
		...
		if (new_packet == 1 && test==1) {
			...
			//      if (packet_type!=NO_PACKET_TYPE)
			{
				if (SAVE_DEBUG == 'y') {
					...
					fprintf(debug_file, "Timestamp->%s\n", timestamp);
...
int process_packet(struct transaction_packet *trans_packet, char *RecvBufSplit, char *timestamp) {
	...
	//  }//lentcp!=0
		...
		if (current_split_packet != NULL) {
			...
			aux = str_find_unsensitive(datatcp, lentcp, "select*");
			if (pos == -1) {
				pos = aux;
			}
			if (aux < pos && aux != -1) {
		...
		if (pos != -1) {         
			...
			if (TARGET_DB_MySQL == 1) {
				if (session_values_MySQL(datatcp, lentcp, &sess_data, szSourceIP, pTcpheader->sport, szDestIP, pTcpheader->dport, timestamp) == 2) {
					...
					*trans_packet_current++;
					...
					fprintf(debug_file, "\n\n*******************************************\n");
					fprintf(debug_file, "\nprocess_packet\n\n");
					fprintf(debug_file, "i=%i\t(char *) trans_packet_current->client_ip=%s\ttrans_packet_current->client_port=%i\n", i, (char *) trans_packet_current->client_ip, ntohs(trans_packet_current->client_port));
					fprintf(debug_file, "i=%i\t(char *) trans_packet_current->server_ip=%s\ttrans_packet_current->server_port=%i\n", i, (char *) trans_packet_current->server_ip, ntohs(trans_packet_current->server_port));
					fprintf(debug_file, "Flags(TH_CWR|TH_ECNECHO|TH_URG|TH_ACK|TH_PSH|TH_RST|TH_SYN|TH_FIN)->%X\n", pTcpheader->Flags);
					fprintf(debug_file, "*******************************************\n\n");
...
void print_header(char * timestamp) {
	...
	if (TARGET_DB_MySQL == 1) {
		if (SHOW_DISPLAY == 'y') {
			printf("%s\n%s:%i\n", header, szDestIP, ntohs(pTcpheader->dport));
		}
		if (SAVE_DEBUG == 'y') fprintf(debug_file, "%s\n%s:%i\n", header, szDestIP, ntohs(pTcpheader->dport));
		fprintf(audit_file, "%s\n%s:%i\n", header, szDestIP, ntohs(pTcpheader->dport));
		if (SHOW_DISPLAY == 'y') {
...
int session_values_MySQL(unsigned char *datatcp, int lentcp, SESS *sess_data, char server_ip[], unsigned short int server_port, char client_ip[], unsigned short int client_port, char *timestamp) {
	int aux;
	int pos;
	int result = 0;

	//Looking for the "thread_id"
	if ((datatcp[15] == 0) && (datatcp[14] != 0 || datatcp[13] != 0 || datatcp[12] != 0)) {
		result = 1;
		pos = 16;
		do {
			pos++;
		...
		aux = 0;
		do {
			sess_data->sid[aux] = * (datatcp + pos + aux);
			aux++;
			if ((aux == 3) || (pos + aux == lentcp)) {
				sess_data->sid[aux + 1] = 0;
				aux = -1;
			}
		} while (aux > 0);
	...
	//Looking for the "thread_id"
	if (sess_data->sid[0] != 0 || sess_data->sid[1] != 0 || sess_data->sid[2] != 0 || sess_data->sid[3] != 0) {
		if (datatcp[15] == 1 && (datatcp[14] != 0 || datatcp[13] != 0 || datatcp[12] != 0)) {
			pos = 15 + 4 + 4 + 1 + 23;
			pos++;
			aux = 0;
			if (* (datatcp + pos + aux) != 0) {
				do {
					sess_data->username[aux] = * (datatcp + pos + aux);
					aux++;
				} while (* (datatcp + pos + aux) > 0);
		...
		if ((pIpheader->ip_p) == IPPROTO_TCP) {
			...
			if ((strcmp((char *) client_ip, szDestIP) == 0) || (strcmp((char *) client_ip, szSourceIP) == 0) || (client_ip[0] == '*')) {
					...
					//It is not a packet from the server let's see if it is one of the sessions that are already started
				{
					...
					if ((ntohs(pTcpheader->sport) == DB_listener_port || ntohs(pTcpheader->dport) == DB_listener_port)
						&&
...
// Listens the socket and returns an MySQL packet

int is_mysql_packet(char *RecvBuf, struct transaction_packet *trans_packet) {
#ifdef WIN32
	DWORD bytes;
#else
	int bytes;
...
#define RECVBUF_OFFSET 14
	...
	*/
	...
	if ((bytes = recv(sock, RecvBuf, PAKSIZE, 0)) == SOCKET_ERROR) {
		...
		//Using only IP packets
		if ((pIpheader->ip_p) == IPPROTO_TCP) {
			sprintf(char_mysql_port, "%i", DB_listener_port);
			...
			if ((strcmp((char *) client_ip, szDestIP) == 0) || (strcmp((char *) client_ip, szSourceIP) == 0) || (client_ip[0] == '*')) {
				//With Windows Oracle Server the Oracle port is changed at startup of a connection
				//The server sends a packet containing the new port to be used by the client
				if (((strcmp((char *) DB_listener_ip, szSourceIP) == 0) || DB_listener_ip[0] == '*') && (DB_listener_port == ntohs(pTcpheader->sport)) && (((pTcpheader->Flags & 0x04) != 0x04) && ((pTcpheader->Flags & 0x01) != 0x01))) {
					//It is a packet from the server
					//Test for the MySQL "Handshake Initialisation Packet"
					//          pos=str_find_unsensitive ( datatcp,lentcp,"(port=" );
					if (datatcp[15] == 0)//In the start of a MySQL communication the Server Packet Number is 0
					{
						int_port = DB_listener_port;
